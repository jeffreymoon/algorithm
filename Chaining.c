...
Node* CHT_CreateNode(KeyType Key, ValueType Value)
{
  ...
  NewNode->Key = (KeyType)malloc(sizeof(KeyType) * strlen(Key) + 1);
  strcpy(NewNode->Key, Key);

  NewNode->Value = (ValueType)malloc(sizeof(ValueType) * strlen(Value) + 1);
...
void CHT_Set(HashTable* HT, KeyType Key, ValueType Value)
{
  ...
  if(HT->Table[Address] == NULL) {
    ...
    NewNode->Next = L;
    HT->Table[Address] = NewNode;
    printf("Collection occured : Key(%s), Address(%d)\n", Key, Address);
  }
}
